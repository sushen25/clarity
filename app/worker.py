...
def process_extract(payload: dict, case_id: str) -> None:
    ...
    for item in result.evidence:
        ...
        db.execute(
            "INSERT INTO evidence_items(id,case_id,source_id,domain,source_location,supporting_text,reporter,setting,confidence,contradiction_status,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
            (new_id(), case_id, source["id"], value["domain"], value["source_location"], value["supporting_text"],
             value["reporter"], value["setting"], value["confidence"], value["contradiction_status"], now()),
        )
...
def process_draft(payload: dict, case_id: str) -> None:
    ...
    evidence = [row_dict(row) for row in get_db().execute(
        "SELECT e.*,s.original_filename FROM evidence_items e JOIN source_documents s ON s.id=e.source_id WHERE e.case_id=? AND e.verified=1", (case_id,)
    )]
...
def _generate_files(draft_id: str) -> None:
    ...
    get_db().execute("UPDATE report_drafts SET docx_path=?,preview_path=?,updated_at=? WHERE id=?",
                     (str(docx_rel), preview_rel, now(), draft_id))
    get_db().commit()
