...
Drafting is split across several bounded requests rather than one. The DIVA symptom sections are drafted alone, and the remaining sections are grouped into `referral`, `background`, `observations`, `findings`, and `synthesis`, each logged under its own `operation=draft_*` record and each receiving only the evidence its sections can use. A single request covering every section, holding the whole evidence ledger, exhausts the output budget on large cases. `synthesis` runs last and receives the sections already drafted, so the summary still overviews the finished report.

Model output grows with the size of the evidence ledger rather than the number of sections, so `background` is normally the largest request. If a group still overflows, its `max_tokens` failure names the group and only that section is left empty, behind a warning the clinician must acknowledge before approval; the rest of the draft survives.

`BEDROCK_DRAFT_MAX_TOKENS` applies per request, so raise it when a group overflows. The binding constraint above roughly 34,000 tokens is `BEDROCK_READ_TIMEOUT_SECONDS`, not the budget: generation has been observed at about 57 output tokens per second, so a larger budget needs a proportionally longer read timeout or the request times out instead of truncating.
