...
Drafting is split across several bounded requests rather than one. The DIVA symptom sections are drafted alone, and the remaining sections are grouped into `narrative`, `findings`, and `synthesis`, each logged under its own `operation=draft_*` record. A single request covering every section exhausts the output budget on evidence-heavy cases, and because a truncated response fails validation immediately the whole draft is lost rather than one part of it. `synthesis` runs last and receives the sections already drafted, so the summary still overviews the finished report. A `max_tokens` failure therefore names the group that overflowed; raise `BEDROCK_DRAFT_MAX_TOKENS`, since the limit applies per request.
