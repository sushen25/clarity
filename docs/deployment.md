...
Model output grows with the size of the evidence ledger, so `BEDROCK_DRAFT_MAX_TOKENS` must accommodate the largest expected case in a single response. Generation has been observed at about 58 output tokens per second, so the budget and `BEDROCK_READ_TIMEOUT_SECONDS` move together: a 40,000-token budget needs roughly 700 seconds of generation and a read timeout comfortably above it. Raising the budget without the timeout converts a truncation into a timeout. The job lock in `app/jobs.py` must also outlast a whole draft, since a request of this length runs well past the original ten-minute window.
