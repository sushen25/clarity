...
The whole report is drafted in one model request, because drafting a section in isolation costs the cross-section context the report depends on. If that request does not fit the output budget or does not parse, the sections are drafted in bounded groups instead (`referral`, `background`, `observations`, `findings`, `synthesis`), each logged under its own `operation=draft_*` record. That fallback is a degraded path: it is recorded as a warning on the draft because the result has less cross-section context. A group that still fails costs its own sections rather than the report. The DIVA symptom sections are always drafted separately, which is a clinical isolation control rather than a size measure.

Drafting requests are streamed. A buffered `converse` returns nothing until the entire response has been generated, which makes the read timeout a wall clock on total generation: a long report then fails outright no matter how high the timeout is set, and raising it only moves the cliff. With `converse_stream` the timeout applies to each chunk, so it detects a genuine stall instead of capping report length.

`BEDROCK_DRAFT_MAX_TOKENS` must still accommodate the largest expected case in a single response, since exceeding it truncates the report and fails the draft. `BEDROCK_READ_TIMEOUT_SECONDS` no longer has to scale with it; it needs only to exceed the gap between chunks, including the initial pause while a large prompt is read.

Bedrock uses the configured read timeout for each non-streaming request and at most two total SDK HTTP attempts. Each attempt emits `bedrock.http_attempt`, making an SDK retry visible without enabling unsafe wire logging. Extraction and drafting use separate output limits so a draft cannot silently inherit an unnecessarily large extraction allowance. The SQLite job queue can still retry a failed job according to its bounded retry policy.
