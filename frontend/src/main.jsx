...
import { criteriaLabels } from "./clinical";
import "./styles.css";
...
function Evidence({data,reload}) {
  const [filter,setFilter]=useState('all'); const items=data.evidence.filter(e=>filter==='all'||e.domain===filter); const domains=[...new Set(data.evidence.map(e=>e.domain))].sort();
  async function update(item,patch){await api(`/cases/${data.case.id}/evidence/${item.id}`,{method:'PATCH',body:JSON.stringify(patch)});reload()}
  return <div><PageTitle eyebrow="EVIDENCE LEDGER" title="Verify extracted evidence" text="Only verified evidence is available to report drafting."/><div className="toolbar"><select value={filter} onChange={e=>setFilter(e.target.value)}><option value="all">All domains</option>{domains.map(d=><option key={d}>{d}</option>)}</select><span>{items.filter(i=>i.verified).length} verified in this view</span></div><div className="evidence-list">{items.map(item=><article className={`evidence-card ${item.verified?'verified':''}`} key={item.id}><div className="evidence-meta"><span>{item.domain.replaceAll('_',' ')}</span><small>{item.reporter||'Unknown reporter'} · {item.setting}</small></div><p>“{item.supporting_text}”</p><div className="evidence-foot"><small>{item.source_location} · confidence {Math.round(item.confidence*100)}%</small><label className="check"><input type="checkbox" checked={item.verified} onChange={e=>update(item,{verified:e.target.checked})}/><span>{item.verified?'Verified':'Verify'}</span></label></div></article>)}</div>{!items.length&&<div className="empty"><ClipboardCheck/><h3>No extracted evidence</h3><p>Upload and process a source document first.</p></div>}</div>;
}

function Criteria({data,reload}) {
  async function change(item,outcome){await api(`/cases/${data.case.id}/criteria/${item.criterion_id}`,{method:'PUT',body:JSON.stringify({...item,clinician_outcome:outcome})});reload()}
  return <div><PageTitle eyebrow="CLINICIAN DECISION" title="Diagnostic criteria" text="The clinician—not the model—sets each criterion outcome."/><Notice>Rating scales are supporting evidence and cannot independently establish a diagnosis.</Notice><div className="criteria-grid">{data.criteria.map(item=><div className="criterion" key={item.criterion_id}><div><strong>{item.criterion_id}</strong><span>{criteriaLabels[item.criterion_id]}</span></div><select className={item.clinician_outcome} value={item.clinician_outcome} onChange={e=>change(item,e.target.value)}><option value="unreviewed">Unreviewed</option><option value="met">Met</option><option value="not_met">Not met</option><option value="insufficient">Insufficient evidence</option></select></div>)}</div></div>;
}

function Report({data,reload}) {
  const draft=data.drafts[0]; const [selectedEvidence,setSelectedEvidence]=useState([]),[busy,setBusy]=useState(false),[error,setError]=useState("");
  ...
  <div className="report-layout"><div className="report-paper">{draft.sections.map((section,si)=><section key={section.key}><h2>{section.heading}</h2>{section.paragraphs.length?section.paragraphs.map((p,pi)=><div key={`${draft.updated_at}-${pi}`} className="draft-paragraph"><textarea defaultValue={p.text} rows={Math.max(3,Math.ceil(p.text.length/105))} disabled={draft.state==='clinician-approved'} onBlur={e=>e.target.value!==p.text&&editParagraph(si,pi,e.target.value)}/><button onClick={()=>setSelectedEvidence(p.evidence_ids)}>{p.evidence_ids.length} source{p.evidence_ids.length===1?'':'s'}</button></div>):<p className="muted">Information not supplied or not yet verified.</p>}</section>)}</div><aside className="evidence-side"><h3>Supporting evidence</h3>{selectedEvidence.length?selectedEvidence.map(id=>{const e=data.evidence.find(x=>x.id===id);return e?<blockquote key={id}>“{e.supporting_text}”<small>{e.reporter||'Unknown'} · {e.setting} · {e.source_location}</small></blockquote>:null}):<p>Select a paragraph’s source count to inspect its evidence.</p>}</aside></div>
