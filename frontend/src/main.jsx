...
function Sources({data,reload}) {
  const [file,setFile]=useState(null),[meta,setMeta]=useState({source_type:"transcript",reporter:"",setting:"unspecified",instrument:""}),[busy,setBusy]=useState(false),[error,setError]=useState(""),[deleting,setDeleting]=useState(null),[editing,setEditing]=useState(null),[confirmingUpload,setConfirmingUpload]=useState(false);
  ...
  async function rename(source,original_filename){setBusy(true);setError("");try{await api(`/cases/${data.case.id}/sources/${source.id}`,{method:'PATCH',body:JSON.stringify({original_filename})});setEditing(null);await reload()}catch(err){setError(err.message);throw err}finally{setBusy(false)}}
...
function RenameSourceDialog({source,busy,onCancel,onSave}) {
  const [filename,setFilename]=useState(source.original_filename),[error,setError]=useState('');
  async function submit(e){e.preventDefault();setError('');try{await onSave(filename.trim())}catch(err){setError(err.message)}}
  return <div className="modal-backdrop" role="presentation"><form className="modal" onSubmit={submit} aria-labelledby="rename-source-title"><div className="eyebrow">SOURCE DOCUMENT</div><h2 id="rename-source-title">Rename document</h2><p className="modal-copy">The file extension is fixed to preserve extraction compatibility.</p>{error&&<Notice tone="danger">{error}</Notice>}<label>File name<input value={filename} onChange={e=>setFilename(e.target.value)} required maxLength="255"/></label><div className="actions"><Button type="button" variant="ghost" onClick={onCancel} disabled={busy}>Cancel</Button><Button disabled={busy}>{busy?'Saving…':'Save name'}</Button></div></form></div>;
}
...
function Evidence({data,reload}) {
  const [filter,setFilter]=useState('all'),[error,setError]=useState(''),[editing,setEditing]=useState(null);
